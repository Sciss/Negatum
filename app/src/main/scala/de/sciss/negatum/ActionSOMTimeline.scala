...
object ActionSOMTimeline extends NamedAction("som-timeline") {
  private def randomRectSides(n: Int)(implicit random: Random): Vec[Vector2] = {
...
//    val dsl = DSL[S]
    ...
    val somIdx = rangeRand(0, numSOM - 1)
...
//        ratioAbove = compressorRatio) * 2
    ...
    val timeNext = tl.lastEvent.getOrElse(0L) + (TimeRef.SampleRate * rangeRand(10, 20)).toLong
