...
object ActionNegatumRecDone extends NamedAction("negatum-rec-done") {
  ...
  def begin[S <: SSys[S]](universe: Universe[S])(implicit tx: S#Tx): Unit = {
    ...
    val neg: Negatum[S] = fLastOpt.collect {
      ...
      _neg.adjustInt   (Negatum.attrBreedElitism   , rangeRand(0, 3))
      _neg.adjustInt   (Negatum.attrBreedGolem     , rangeRand(15, 25))
      _neg.adjustInt   (Negatum.attrBreedMaxMut    , rangeRand(4 , 6))
      _neg.adjustDouble(Negatum.attrBreedProbMut   , rangeRand(0.73, 0.76))

      _neg.adjustDouble(Negatum.attrEvalMaxBoost   , rangeRand(11.0, 13.0))
      _neg.adjustDouble(Negatum.attrEvalTimeWeight , rangeRand(0.3, 0.5))

      _neg.adjustInt   (Negatum.attrGenMinVertices , rangeRand(8, 12))
      _neg.adjustInt   (Negatum.attrGenMaxVertices , rangeRand(80, 100))
      _neg.adjustInt   (Negatum.attrGenPopulation  , 1000)
      _neg.adjustDouble(Negatum.attrGenProbConst   , rangeRand(0.4, 0.5))
